...
DOCUMENTATION = '''
---
module: get_mysql_config
short_description: get mysql conf
'''
...
def get_mysql_config(**param):
    ...
    # datadir
    datadir=config.get(section).get("datadir") if "datadir" in config.get(section).keys() else '/var/lib/mysql'
    # socket    
    mysql_socket=config.get(section).get("socket") if "socket" in config.get(section).keys() else '/tmp/mysql.sock'
    if mysql_socket[:1]<>'/':
        mysql_socket=os.path.join(datadir,'mysql.sock')
    # error log
    if "log_error" in config.get(section).keys():
        errlog=config.get(section).get("log_error")
    ...
    # slow log
    if "slow_query_log" in config.get(section).keys():
        slowlog=config.get(section).get("slow_query_log_file") if "slow_query_log_file" in config.get(section).keys() else socket.gethostname()+'-slow.log'
    else:
        slowlog=''
    if slowlog and slowlog[:1]<>'/':
        slowlog=os.path.join(datadir,slowlog)
    # audit log
    if "audit_json_log_file" in config.get(section).keys():
        auditlog=config.get(section).get("audit_json_log_file") if "audit_json_log_file" in config.get(section).keys() else 'mysql-audit.json'
    else:
        auditlog=''
    if auditlog and auditlog[:1]<>'/':
        auditlog=os.path.join(datadir,auditlog)
    else:
        auditlog=os.path.join(datadir,'mysql-audit.json')
    result['datadir']=datadir
    result['socket']=mysql_socket
    result['log_error']=errlog
    result['slow_query_log_file']=slowlog
    result['audit_json_log_file']=auditlog
    return result
...
def main():
    ...
    result = get_mysql_config(**module.params)
